...
export function defineCycle(c: Ciclo): Ciclo {
  ...
  if (c.metodo === 'incremental_watermark' && c.janela !== 'desde_watermark') {
    throw new Error(`Ciclo ${c.id}: incremental por watermark exige janela desde_watermark.`)
  }
...
export function janelaDeLeitura(watermark: Date | null, agora: Date): { de: Date; ate: Date } {
  return {
    de: watermark ? new Date(watermark.getTime() - SOBREPOSICAO_MS) : new Date(0),
    ate: agora,
  }
}
