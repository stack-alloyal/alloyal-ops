...
import { type Alarme, contarFalhasConsecutivas, executarCiclo, gravarWatermark, lerWatermark } from './runner.js'
...
describe('executor de ciclos', { skip: !ADMIN }, () => {
  ...
  test('sucesso registra a execução e avança o watermark', async () => {
    ...
    const r = await executarCiclo(
      ciclo('T-ok', async () => ({ linhasLidas: 120, linhasGravadas: 118, novoWatermark: novo })),
      deps(),
    )
    ...
    assert.equal((await lerWatermark(pool, 'T-ok'))?.toISOString(), novo.toISOString())
  })
  ...
  test('falha NÃO avança o watermark: a janela fica intacta', async () => {
    ...
    assert.equal((await lerWatermark(pool, 'T-falha'))?.toISOString(), wm.toISOString())
  ...
  test('repete conforme a política e só então desiste', async () => {
    ...
    const { rows } = await pool.query(`SELECT count(*)::int n FROM ops.cycle_run WHERE ciclo = 'T-retry'`)
  ...
  test('alarma só depois de N execuções seguidas falhando', async () => {
    ...
    assert.equal(r1.estado === 'falha' && r1.alarmado, false, 'primeira falha não acorda ninguém')
    assert.equal(alarmes.length, 0)
  ...
  test('o mesmo ciclo não roda concorrente consigo mesmo', async () => {
    ...
    assert.equal(concorrente.estado === 'pulado' && concorrente.motivo, 'em_execucao')
