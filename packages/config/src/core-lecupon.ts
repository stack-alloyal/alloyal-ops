...
export function credencialDoAmbiente(env: NodeJS.ProcessEnv): CredencialDoCore | null {
  const token = (env['LECUPON_CLIENT_EMPLOYEE_TOKEN'] ?? '').trim()
  const email = (env['LECUPON_CLIENT_EMPLOYEE_EMAIL'] ?? '').trim()
  if (!token || !email) return null
  const base = (env['LECUPON_API_BASE'] ?? 'https://api.lecupon.com/client/v3').replace(/\/$/, '')
  ...
  return tenant ? { base, token, email, tenantCnpj: tenant } : { base, token, email }
}
...
export function modulosDe(n: NegocioDoCore): { modulo: string; ativo: boolean }[] {
...
export function cnpjNormalizado(bruto: string | null | undefined): string | null {
...
export async function lerNegocios(
  ...
  for (; pagina <= maxPaginas; pagina++) {
    const resp = await buscar(`${c.base}/businesses?page=${pagina}&per_page=30`, {
      headers: cabecalhos(c),
      signal: AbortSignal.timeout(20_000),
    })
    if (!resp.ok) throw new CoreIndisponivelError(resp.status, await resp.text())

    const lista = extrairLista(await resp.json())
    if (lista.length === 0) return { negocios, paginas: pagina - 1, parcial: false }
