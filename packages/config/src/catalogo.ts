...
export const CATALOGO: readonly Ajuste[] = [
  ...
  },
  {
    ...
    porQueOLimite:
      "Mesmo motivo do mínimo de empresas: 50 é piso, não sugestão.",
    lidoEm: "packages/success/src/benchmark.ts",
  },
  {
    ...
    unidade: "meses",
    efeito:
      "Quantos meses aparecem na evolução do relatório. Mais meses, mais páginas no PDF.",
...
export const SEGREDOS: readonly Segredo[] = [
  {
    ...
    ondeConseguir:
      "HubSpot → Configurações → Integrações → Private Apps → escopo de leitura de deals",
    semEle:
      "Os ciclos C4 e C5 não rodam: contratos e eventos de MRR não entram.",
    irrecuperavel:
  ...
  },
  {
    ...
    semEle:
      "O webhook do C5 é recusado: sem verificar assinatura, qualquer um posta evento de MRR falso.",
  },
  {
    ...
    semEle:
      "O ciclo C6 não roda e o sinal de engajamento (S-ENG) fica neutro e sinalizado.",
    irrecuperavel:
  ...
  },
  {
    ...
    ondeConseguir:
      "CleverTap → Settings → Project — é o prefixo do host da sua conta (us1, eu1, in1, sg1, aps3)",
    semEle:
  ...
  },
  {
    ...
    semEle:
      "O ciclo C8 não roda: adimplência não entra, e o sinal financeiro fica neutro.",
  },
  {
  ...
  {
    chave: "lecupon.employee_token",
    rotulo: "Token de employee da API do core (Lecupon v3)",
    ondeConseguir:
      "Painel do cliente Lecupon → sessão de um usuário com acesso de administração. " +
      "É o `X-ClientEmployee-Token` das chamadas de api.lecupon.com/client/v3.",
    semEle:
      "O ciclo C18 fica INERTE: nenhum cadastro de cliente entra e core.account continua " +
      "vazia. Sem cadastro de cliente, nada que depende de conta tem o que mostrar.",
  },
  {
    chave: "lecupon.employee_email",
    rotulo: "E-mail de employee da API do core",
    ondeConseguir:
      "O e-mail da MESMA conta que gerou o token — a API exige o par.",
    semEle:
      "O C18 não autentica: o token sozinho não abre a API. Vai no cabeçalho " +
      "`X-ClientEmployee-Email`.",
  },
  {
    chave: "lecupon.tenant_cnpj",
    rotulo: "CNPJ do tenant no core",
    ondeConseguir: "O CNPJ da própria Alloyal no core, só dígitos.",
    semEle:
      "A API responde no escopo do tenant padrão da credencial. Se a conta atender mais de " +
      "um tenant, a leitura vem do tenant errado — e o C18 gravaria cadastro de outra base " +
      "sem nada indicar o erro.",
  },
  {
    chave: "replica.url",
  ...
  },
  {
    ...
    semEle:
      "O relatório é composto, revisado e congelado, mas o envio é manual.",
  },
];
