...
describe("de-para com o HubSpot", { skip: !ADMIN }, () => {
  ...
  const limpar = async () => {
    await db.query(
      "UPDATE core.account SET parent_account_id = NULL WHERE brand_id LIKE 'zzt-%'",
    );
  ...
  test("vínculo NÃO ambíguo vai para a coluna única", async () => {
    const r = await sincronizarCadastro(
      db,
      [neg("zzt-1", { hubspot_company_id: "8100000001" })],
      AGORA,
      false,
    );
    ...
    assert.equal(rows[0].hubspot_company_id, "8100000001");
  });

  test("id que NÃO é inteiro positivo nunca entra no de-para", async () => {
    // Havia duas contas reais com `hubspot_company_id = '0'`, e as duas apareciam
    // ligadas entre si na view de ambíguos — MEGA PROTEGE e uma associação de socorro
    // mútuo, que nada têm em comum. Zero é o nulo do core vazando como texto.
    for (const invalido of ["0", "hs-antigo", "", "12.5"]) {
      await sincronizarCadastro(
        db,
        [neg("zzt-9", { hubspot_company_id: invalido })],
        AGORA,
        false,
      );
      const { rows } = await db.query(
        `SELECT count(*)::int AS n FROM core.account_hubspot h
           JOIN core.account a ON a.id = h.account_id WHERE a.brand_id = 'zzt-9'`,
      );
      assert.equal(rows[0].n, 0, `"${invalido}" não deveria virar vínculo`);
    }
  });
  ...
  test("vínculo AMBÍGUO não vai para a coluna única, e a carga NÃO quebra", async () => {
    ...
    const r = await sincronizarCadastro(
      db,
      [
        neg("zzt-2", { hubspot_company_id: "8100000002" }),
        neg("zzt-3", { hubspot_company_id: "8100000002" }),
      ],
      AGORA,
      false,
    );
  ...
  test("mas o de-para GUARDA o vínculo ambíguo — nada se perde", async () => {
    const { rows } = await db.query(
      `SELECT count(*)::int n FROM core.account_hubspot h
        ...
        WHERE a.brand_id IN ('zzt-2','zzt-3') AND h.hubspot_company_id = '8100000002'`,
    );
  ...
  test("a view aponta o caso que precisa de decisão humana", async () => {
    const { rows } = await db.query(
      "SELECT contas, raizes FROM core.hubspot_ambiguo WHERE hubspot_company_id = '8100000002'",
    );
    assert.equal(rows[0].contas, "2");
    assert.equal(
      rows[0].raizes,
      "2",
      "duas RAÍZES: não se explica por matriz/filial",
    );
  });
  ...
  test("módulo que o core deixou de reportar SAI", async () => {
    await sincronizarCadastro(
      db,
      [neg("zzt-4", { cashback: true, giftcard: true })],
      AGORA,
      false,
    );
  ...
  test("cliente ausente é INATIVADO, e nunca apagado", async () => {
    ...
    const { rows } = await db.query(
      "SELECT ativo FROM core.account WHERE brand_id = 'zzt-4'",
    );
  ...
  test("o banco RECUSA apagar conta, com mensagem que ensina o caminho", async () => {
    ...
    const { rows } = await db.query(
      "SELECT count(*)::int n FROM core.account WHERE brand_id = 'zzt-4'",
    );
  ...
  test("a hierarquia liga filial à matriz", async () => {
    ...
    await sincronizarCadastro(
      ...
      [
        neg("zzt-10"),
        neg("zzt-11", { main_business_id: "zzt-10" as unknown as number }),
      ],
      depois,
      false,
    );
