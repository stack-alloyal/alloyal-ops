 ...
 */
export function contaDeServicoDoAmbiente(env: NodeJS.ProcessEnv): ContaDeServico | null {
  const json = (env['GOOGLE_SA_JSON'] ?? '').trim()
  if (json) {
    try {
      const o = JSON.parse(json) as Partial<ContaDeServico>
      if (o.client_email && o.private_key) {
        return { client_email: o.client_email, private_key: normalizarChave(o.private_key) }
      }
    } catch {
  ...
  if (email && chave.trim()) return { client_email: email, private_key: normalizarChave(chave) }
  return null
}
...
export class ClienteGmail {
  ...
  static async porContaDeServico(
    ...
    if (!resp.ok) {
      ...
      throw new Error(`${resp.status}: ${traduzirErroDeToken(corpo, assunto, sa.client_email)}`)
    }
 ...
 */
export function traduzirErroDeToken(corpo: string, assunto: string, saEmail: string): string {
