...
import { contaDeServicoDoAmbiente, traduzirErroDeToken, ESCOPO_ENVIO } from './gmail.js'
import {
  codificar2047,
...
const CHAVE_FALSA = '-----BEGIN PRIVATE KEY-----\nMIIB\n-----END PRIVATE KEY-----\n'

const cfg: ConfiguracaoDeEnvio = {
  conta: { client_email: 'sa@projeto.iam.gserviceaccount.com', private_key: CHAVE_FALSA },
...
test('o remetente padrão é Alloyal Pulse <noreply@alloyal.com.br>', () => {
  ...
  const c = configuracaoDoAmbiente({ GOOGLE_SA_JSON: JSON.stringify(cfg.conta) })
  assert.ok(c)
...
test('aceita as duas formas da casa: JSON inteiro e par separado', () => {
  const porJson = contaDeServicoDoAmbiente({ GOOGLE_SA_JSON: JSON.stringify(cfg.conta) })
...
test('JSON quebrado devolve null em vez de derrubar', () => {
  assert.equal(contaDeServicoDoAmbiente({ GOOGLE_SA_JSON: '{isso nao e json' }), null)
})

test('JSON válido sem as chaves certas devolve null', () => {
  assert.equal(contaDeServicoDoAmbiente({ GOOGLE_SA_JSON: '{"type":"service_account"}' }), null)
})

// ─── MIME ────────────────────────────────────────────────────────────────────

const decodificar = (raw: string) => Buffer.from(raw, 'base64url').toString('utf8')

test('o MIME traz From, To e Subject', () => {
  const m = decodificar(montarMime({ para: 'a@alloyal.com.br', assunto: 'Oi', html: '<p>x</p>' }, cfg))
...
test('assunto com acento é codificado em RFC 2047', () => {
  ...
  const m = decodificar(montarMime({ para: 'a@alloyal.com.br', assunto: 'Seu código', html: '<p>x</p>' }, cfg))
...
test('o corpo vai em base64 com linhas de no máximo 76 caracteres', () => {
  ...
  const corpo = decodificar(montarMime({ para: 'a@alloyal.com.br', assunto: 'x', html }, cfg)).split('\r\n\r\n')[1]!
...
test('Reply-To só aparece quando existe', () => {
  const sem = decodificar(montarMime({ para: 'a@alloyal.com.br', assunto: 'x', html: 'y' }, cfg))
  assert.doesNotMatch(sem, /^Reply-To:/m)
  const com = decodificar(
    montarMime({ para: 'a@alloyal.com.br', assunto: 'x', html: 'y' }, { ...cfg, responderPara: 'noreply@alloyal.com.br' }),
  )
...
test('unauthorized_client é traduzido para "falta delegação", não "credencial errada"', () => {
  ...
  // errado custa horas.
  const t = traduzirErroDeToken('{"error":"unauthorized_client"}', 'noreply@alloyal.com.br', 'sa@x.iam')
  assert.match(t, /DELEGAÇÃO/)
  assert.match(t, new RegExp(ESCOPO_ENVIO.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
})

test('invalid_grant lista as causas na ordem em que acontecem', () => {
  const t = traduzirErroDeToken('{"error":"invalid_grant"}', 'noreply@alloyal.com.br', 'sa@x.iam')
...
test('href que não seja http, https ou mailto vira #', () => {
  ...
  assert.equal(hrefSeguro('https://pulse.alloyal.com.br/x'), 'https://pulse.alloyal.com.br/x')
  assert.equal(hrefSeguro('mailto:a@alloyal.com.br'), 'mailto:a@alloyal.com.br')
})

test('aspas na URL não quebram o atributo', () => {
  assert.equal(hrefSeguro('https://x/"onmouseover="alert(1)'), 'https://x/&quot;onmouseover=&quot;alert(1)')
})
